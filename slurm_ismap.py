...
def parse_args():
    ...
    parser.add_argument('--reads', type=str, required=True, help='Paired end read files in fastq.gz format')
    ...
    parser.add_argument('--assemblies', type=str, required=False, help='Contig assemblies, one for each read set (If using improvement option)')
...
def main():
    ...
    for sample in fileSets:
        ...
        if args.assemblies:
            cmd += " --assemblies " + args.assemblies
        if args.assemblyid:
